...
<body>
    ...
    <section class="section-main container p-0">
            <div id="profile-header" class="px-2 d-flex align-items-center sticky-top">
                <a href="../controller/IndexController.php" class="bg-transparent btn-plus" style="font-weight: bold;">
                    <div class="arrow-bg">
                ...
                <div class="d-flex flex-column justify-content-center ms-4">
                    <p class="fw-bold m-0"><?= $profileUser->getUsername()?></p>
                    <p class="m-0" id="post-n"><?=count($profileUser->getTweets())." posts"?></p>
                </div>
            </div>
        <header>
            ...
            <div>
                <img id="profile-pic" class="rounded-circle m-3" height="40px" src="../img/icon-profile.png">
                <?php if (($profileUser->getId() === $userLogged->getId())):?>
                    <a href="../controller/EditController.php?id=<?=$userLogged->getId()?>" class="text-decoration-none ms-auto m-3 p-2 btn-perfil d-flex align-items-center justify-content-center" data-bs-toggle="modal" data-bs-target="#modalEdit">Editar</a>
                <?php elseif (!$follows) :?>
                    <a href="../controller/FollowController.php?id=<?=$profileUser->getId()?>" class="text-decoration-none ms-auto m-3 p-2 btn-perfil d-flex align-items-center justify-content-center">Seguir</a>
                <?php else :?>
                    <a href="../controller/UnfollowController.php?id=<?=$profileUser->getId()?>" class="btn-unfollow text-decoration-none ms-auto m-3 p-2 btn-perfil d-flex align-items-center justify-content-center"><span id="following">Siguiendo</span><span id="unfollow">Dejar de seguir</span></a>
                <?php endif ?>
            </div>
            <div class="p-3 d-flex flex-column">
                <p class="m-0 fw-bold fs-5"><?=$profileUser->getUsername()?></p>
                <p class="mx-0 mt-2 mb-1"><?=$profileUser->getDescription()?></p>
                <div class="following d-flex flex-row gap-2">
                    <a href="../controller/FollowersTabController.php?id=<?=$profileUser->getId()?>" class="tw-name" id="follows"><p><span class="fw-bold"><?=count($profileUser->getfollowingId())." "?></span>Siguiendo</p></a>
                    <a href="../controller/FollowersTabController.php?id=<?=$profileUser->getId()?>" class="tw-name" id="followers"><p><span class="fw-bold"><?=count($profileUser->getfollowersId())." "?></span>Seguidores</p></a>
                </div>
            </div>
            <hr class="m-0"/>
        </header>
        ...
        ?>
        <?php foreach($profileUser->getTweets() as $tweet) :?>
            <div class="tweet p-3">
                <div class="d-flex flex-row">
                    <a class="me-2" href="../controller/ProfileController.php?id=<?= $tweet->getUser()->getId() ?>">
                        <img class="rounded-circle" height="40px" src="../img/icon-profile.png">
                    </a>
                    <a class="tw-name my-0 fw-bold" href="../controller/ProfileController.php?id=<?= $tweet->getUser()->getId() ?>"><?=$tweet->getUser()->getUsername()?></a>
                    <p class="my-0 mx-2">·</p>
                    <p class="my-0">1h</p>
                    ...
                    <div class="btn-group dropstart ms-auto">
                        ...
                        <ul class="dropdown-menu">
                            <?php if ($profileUser->getId() === $userLogged->getId()) :?>
                                <li>
                                    <a class="delete-tweet fw-bold p-2 text-decoration-none" href="DeleteController.php?id=<?=$tweet->getId()?>">
                ...
                <div>
                    <p class="ms-5 mb-2"><?=$tweet->getText()?></p>
                </div>
            </div>
            <hr class="m-0">
        <?php endforeach ?>
...
<!-- Tweet: Modal -->
        ...
        <!--<div class="modal show" id="modalTweet" tabindex="-1" aria-labelledby="modalTweetLabel" style="display: block;" aria-modal="true">-->
            <div class="modal-dialog modal-fullscreen-md-down">
                <div class="modal-content">
                    <form action="TwittearController.php" method="POST" >
                        <div class="modal-header">
                            <button type="button" class="bg-transparent" style="font-weight: bold;" data-bs-dismiss="modal" aria-label="Close">
                                <svg viewBox="0 0 24 24" class="arrow"><g><path d="M7.414 13l5.043 5.04-1.414 1.42L3.586 12l7.457-7.46 1.414 1.42L7.414 11H21v2H7.414z"></path></g></svg>
                            </button>
